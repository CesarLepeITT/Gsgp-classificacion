...
int main(int argc, char **argv)
{
    ...
    if (!trainFile_s.empty() && testFile_s.empty())
    {
        ...
        // Checa las dimensiones de la data
        ...
        float executionTime = 0,
              initialitionTimePopulation = 0,
              timeComputeSemantics = 0,
              generationTime = 0; /*!< Variables that store the time in milliseconds between the events mark1 and mark2.*/
        ...
        ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        /*
            aki hago la cosa que me dijo el mane para ver que la activation function no se muera


        */
        float *ucpySemanticTrainCases;
        int nbytes;
        nbytes = sizeof(uSemanticTrainCases);

        cudaMallocManaged((void **)&ucpySemanticTrainCases, nbytes);
        cudaMemcpy(ucpySemanticTrainCases, uSemanticTrainCases, nbytes, cudaMemcpyDeviceToDevice);
        printf("Alpha usado lmao %f", config.alpha);
        ActivationFunction<<<gridSize, blockSize>>>(uSemanticTrainCases, ucpySemanticTrainCases, config.alpha, nrow, config.populationSize);

        float *hcpySemanticTrainCases;
        hcpySemanticTrainCases = (float *)malloc(nbytes);
        cudaMemcpy(hcpySemanticTrainCases, ucpySemanticTrainCases, nbytes, cudaMemcpyDeviceToHost);

        saveSemantics("/home/gallobota/Escritorio/Gsgp-classificacion", hcpySemanticTrainCases, config.populationSize, "saldia", nvar, 0);
        ...
        if (config.errorFunction == 0)
        {
            ...
            // Sigmoide aki
            ActivationFunction<<<gridSize, blockSize>>>(uSemanticTrainCases, config.alpha, nrow, config.populationSize);
            cudaDeviceSynchronize();
            */
        ...
        for (int generation = 1; generation <= config.numberGenerations; generation++)
        {
            ...
            if (config.errorFunction == 0)
            {
                ...
                /*
                    ...
                    calcular el salto sigmoidal ver que siempre sea correcto por el interprete con valores muy grandes

                    semantic train cases

                    compute semantics despues de dos semantics hgacer kenrenel que apllique a cada elemento de vector

                    cuando calcule el rmse mandar la nueva rmsea

                    save semantics devolver esa matriz al contexto de la cpu para ver si la sigmoidal si esta reduciendo los valores
                    semanticos de cada vector

                    semantica al contexto de cpu usarla para revisar la semantica,
                    nomas transformar la semantica

                */

                //   ActivationFunction<<<gridSize, blockSize>>>(uSemanticTrainCasesNew, config.alpha, nrow, config.populationSize);
                // cudaDeviceSynchronize();
                /*!< invokes the GPU to calculate the error (RMSE) the initial population*/
